...
interface BaseResult<T, E> extends Iterable<T extends Iterable<infer U> ? U : never> {
   ...
   */
  andThen: (<T2>(mapper: (val: T) => Ok<T2>) => Result<T2, E>) & (<E2>(mapper: (val: T) => Err<E2>) => Result<T, E | E2>) & (<T2, E2>(mapper: (val: T) => Result<T2, E2>) => Result<T2, E | E2>) & (<T2, E2>(mapper: (val: T) => Result<T2, E2>) => Result<T2, E | E2>)
   ...
   * Match `Result<T>`
   */
  match: <R>(matcher: Matcher<T, E, R>) => R
}
...
export class ErrImpl<E> implements BaseResult<never, E> {
  ...
  match<R>(matcher: Matcher<never, E, R>): R {
...
export class OkImpl<T> implements BaseResult<T, never> {
  ...
  match<R>(matcher: Matcher<T, never, R>): R {
