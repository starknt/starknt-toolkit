...
export class Map<const Item, Output, I extends Iterator<Item> = Iterator<Item>, F extends (item: Item) => Output = (item: Item) => Output> implements IntoIterator<Output> {
  protected iter: I
  protected f: F
  ...
  try_fold<B, R extends Option<B> = Option<B>, Fold extends (b: B, item: Output) => R = (b: B, item: Output) => R>(init: B, f: Fold): R {
    return this.iter.try_fold(init, map_try_fold(this.f, f) as any) as R
  }

  fold<B, Fold extends (b: B, item: Output) => B = (b: B, item: Output) => B>(init: B, f: Fold): B {
    return this.iter.fold(init, map_fold(this.f, f))
  }
  ...
  unzip<A, B>(this: Map<[A, B], [A, B], any, any>): [A[], B[]] {
    const left: A[] = []
    const right: B[] = []
    let item: Option<[A, B]>
    while ((item = this.next()) && item.isSome()) {
